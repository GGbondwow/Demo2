    ...
    void Update()
    {
        ...
        if(info.atkType == 1)
        {
                ...
                Time.time - nowTime >= info.offsetTime)
            {
                ...
                //创建开火特效
                GameObject effObj = Instantiate(Resources.Load<GameObject>(info.eff),gunPoint.position,gunPoint.rotation * Quaternion.AngleAxis(90,Vector3.right));
                //延迟移除特效
                Destroy(effObj,0.5f);
                //记录开火时间
                nowTime = Time.time;
